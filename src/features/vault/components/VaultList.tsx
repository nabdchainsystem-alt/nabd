...
const getItemIcon = (type: string, title: string = '') => {
    switch (type) {
        case 'folder': return <Folder size={16} className="fill-blue-500 text-blue-600 dark:text-blue-400" />;
        ...
        case 'weblink': return <ExternalLink size={16} className="text-cyan-500" />;
                    ...
                    {items.map(item => (
                        ...
                        >
                            ...
                            <td className="py-3 px-4 text-sm text-gray-500 dark:text-gray-400">
                                {item.type === 'weblink' ? (
                                    <a href={(item.metadata as any)?.url} target="_blank" rel="noopener noreferrer" className="text-blue-500 hover:underline flex items-center gap-1" onClick={e => e.stopPropagation()}>
