...
import { MagnifyingGlass as Search, X } from 'phosphor-react';
...
const DropdownPicker: React.FC<DropdownPickerProps> = ({ options, onSelect, onClose, currentId, darkMode }) => {
...
export const DropdownCell: React.FC<any> = ({ value, onChange, options, darkMode }) => {
    ...
    return (
        <div className="w-full h-full p-1 relative">
            <button
                ...
                className={`w-full h-full rounded flex items-center justify-start px-2 hover:opacity-80 transition-all ${selectedOption
                    ? 'text-white font-medium'
                    : 'text-gray-400 hover:bg-gray-100 dark:hover:bg-stone-800/50'
                    }`}
                style={{
