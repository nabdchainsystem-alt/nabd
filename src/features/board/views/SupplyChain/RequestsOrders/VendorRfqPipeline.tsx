...
import { Users, MessageSquare, Clock, FileText, CheckCircle, AlertCircle, DollarSign, ShieldAlert } from 'lucide-react';

export const VendorRfqPipeline: React.FC = () => {
    // R04: Vendor & RFQ Pipeline
    const kpis = [
        { title: "RFQs Sent", value: "42", trend: "Last 30 days", trendDirection: "neutral", icon: <FileText size={20} />, color: "blue" },
    ...
    const rfqsByStatus = {
        ...
        yAxis: { type: 'value' },
        series: [
            { name: 'RFQs', type: 'bar', data: [5, 12, 8, 10, 7], itemStyle: { color: '#3b82f6' } }
        ]
    };
    ...
    const tableColumns = [
        ...
        {
            header: "Best Price", accessor: "best_price_flag", render: (val: string) => (
                <span className={`px-2 py-1 rounded-full text-xs font-semibold ${val === 'Yes' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'}`}>{val}</span>
            )
        }
    ];

    const tableData = [
        { rfq_id: "RFQ-9001", vendor_name: "TechSolutions Inc", item_code: "IT-Servers", requested_qty: "5", rfq_status: "Sent", quote_status: "Received", response_time_hours: "18", best_price_flag: "Yes" },
        { rfq_id: "RFQ-9001", vendor_name: "Global IT", item_code: "IT-Servers", requested_qty: "5", rfq_status: "Sent", quote_status: "Received", response_time_hours: "48", best_price_flag: "No" },
        { rfq_id: "RFQ-9002", vendor_name: "Safety First Co", item_code: "PPE-Gloves", requested_qty: "2500", rfq_status: "Sent", quote_status: "Pending", response_time_hours: "-", best_price_flag: "-" },
        { rfq_id: "RFQ-9003", vendor_name: "Office Depot", item_code: "Stationery", requested_qty: "500", rfq_status: "Closed", quote_status: " awarded", response_time_hours: "24", best_price_flag: "Yes" },
        { rfq_id: "RFQ-9004", vendor_name: "BuildMat", item_code: "Cement", requested_qty: "1000", rfq_status: "Sent", quote_status: "Late", response_time_hours: "120", best_price_flag: "No" },
    ];
    ...
    return (
        <div className="flex flex-col h-full bg-gray-50 dark:bg-[#09090b] p-6 overflow-y-auto">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8 h-96">
                <div className="lg:col-span-2 h-full"><DashboardChart title="RFQs by Status" options={rfqsByStatus} height="100%" /></div>
                <div className="h-full"><DashboardChart title="Response Time Distribution" options={responseTimeDist} height="100%" /></div>
            </div>
