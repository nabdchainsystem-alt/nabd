...
const SimpleSortableTable = ({ columns, data, onSort, sortConfig }: { columns: any[], data: any[], onSort: (key: string) => void, sortConfig: { key: string, direction: 'asc' | 'desc' } | null }) => (
                    ...
                    {columns.map((col, idx) => (
                        ...
                        >
                            <div className="flex items-center gap-1">
                                {col.header}
                                <ArrowUpDown size={12} className={`opacity-0 group-hover:opacity-50 ${sortConfig?.key === col.accessor ? 'opacity-100 text-blue-500' : ''}`} />
                            </div>
                        </th>
                    ))}
                </tr>
...
export const DailyRequestsControl: React.FC = () => {
    ...
    const [requestRows, setRequestRows] = useState([
        { time: "14:30", event: "Large PO Approval", request_id: "PR-901", user: "Director", department: "Ops", amount: "$8,500", status: "Approved" },
        { time: "13:15", event: "Emergency PR", request_id: "PR-902", user: "Site Mgr", department: "IT", amount: "$1,200", status: "Critical" },
        { time: "11:00", event: "Batch Process", request_id: "-", user: "System", department: "All", amount: "-", status: "Completed" },
        { time: "10:45", event: "Budget Alert", request_id: "PR-899", user: "Finance", department: "Marketing", amount: "$12k", status: "Flagged" },
        { time: "09:00", event: "New Vendor", request_id: "-", user: "Officer", department: "Procurement", amount: "-", status: "Completed" },
    ]);
    ...
    const tableColumns = [
        ...
        { header: "Amount", accessor: "amount" },
        {
            header: "Status", accessor: "status", render: (val: string) => (
                <span className={`px-2 py-1 rounded-full text-xs font-semibold ${val === 'Critical' || val === 'Flagged' ? 'bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-400' :
                    val === 'Approved' || val === 'Completed' ? 'bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400' :
    ...
    const kpis = [
        { title: "Requests Today", value: "24", trend: "Normal Volume", trendDirection: "neutral", icon: <Calendar size={20} />, color: "blue" },
        { title: "Active Requests", value: "142", trend: "+5 from yesterday", trendDirection: "up", icon: <Activity size={20} />, color: "indigo" },
        { title: "Approvals Today", value: "18", trend: "Processing", trendDirection: "up", icon: <CheckCircle size={20} />, color: "green" },
        { title: "Spend Committed", value: "$45.2k", trend: "Today", trendDirection: "neutral", icon: <DollarSign size={20} />, color: "purple" },
        { title: "Overdue Requests", value: "12", trend: "+2 vs avg", trendDirection: "down", icon: <Clock size={20} />, color: "orange" },
        { title: "Critical Alerts", value: "3", trend: "Action Required", trendDirection: "down", icon: <AlertCircle size={20} />, color: "red" },
        { title: "Avg Cycle Time", value: "3.5 Days", trend: "-0.5 days", trendDirection: "up", icon: <Activity size={20} />, color: "teal" },
        { title: "SLA Compliance", value: "98.5%", trend: "Target Met", trendDirection: "up", icon: <CheckCircle size={20} />, color: "green" },
    ];
    ...
    const requestsByStatus = {
        ...
        yAxis: { type: 'value' },
        series: [
            { name: 'Requests', type: 'bar', data: [5, 42, 28, 65, 2], itemStyle: { color: '#3b82f6' } }
        ]
    };
    ...
    return (
        <div className="flex flex-col h-full bg-gray-50 dark:bg-[#09090b] p-6 overflow-y-auto">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8 h-96">
                <div className="lg:col-span-2 h-full"><DashboardChart title="Hourly Activity" options={requestsHourly} height="100%" /></div>
                <div className="h-full"><DashboardChart title="Requests by Status" options={requestsByStatus} height="100%" /></div>
            </div>
