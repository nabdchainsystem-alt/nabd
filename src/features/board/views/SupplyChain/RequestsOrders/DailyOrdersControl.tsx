...
export const DailyOrdersControl: React.FC = () => {
    ...
    const kpis = [
        { title: "Open Orders", value: "342", trend: "High Volume", trendDirection: "up", icon: <Package size={20} />, color: "blue" },
        { title: "Created Today", value: "45", trend: "+12 vs avg", trendDirection: "up", icon: <Calendar size={20} />, color: "green" },
        { title: "Due Today", value: "28", trend: "Deadline", trendDirection: "neutral", icon: <Clock size={20} />, color: "orange" },
        { title: "Overdue Orders", value: "12", trend: "Critical", trendDirection: "down", icon: <AlertTriangle size={20} />, color: "red" },
        { title: "Blocked Orders", value: "8", trend: "Action Required", trendDirection: "down", icon: <ShieldAlert size={20} />, color: "red" },
        { title: "Avg Cycle Time", value: "2.4 Days", trend: "Fast", trendDirection: "up", icon: <PlayCircle size={20} />, color: "blue" },
        { title: "OTIF Today", value: "94%", trend: "Target Missed", trendDirection: "down", icon: <CheckCircle size={20} />, color: "yellow" },
        { title: "Critical Alerts", value: "3", trend: "New", trendDirection: "down", icon: <AlertTriangle size={20} />, color: "red" },
    ];
        ...
        { header: "Value", accessor: "value" },
        {
            header: "Status", accessor: "status", render: (val: string) => (
                <span className={`px-2 py-1 rounded-full text-xs font-semibold ${val === 'Critical' ? 'bg-red-100 text-red-700' :
                    val === 'Shipped' ? 'bg-green-100 text-green-700' : 'bg-blue-100 text-blue-700'}`}>
                    {val}
                </span>
            )
        }
    ...
    const tableData = [
        ...
        { time: "15:30", order_id: "ORD-4998", event: "Order Shipped", user: "Warehouse", value: "$5,000", status: "Shipped" },
        { time: "14:15", order_id: "ORD-4800", event: "Hold Applied", user: "Credit Control", value: "$12,000", status: "Critical" },
        { time: "13:00", order_id: "ORD-5000", event: "Order Picked", user: "Warehouse", value: "$800", status: "Processing" },
    ...
    return (
        <div className="flex flex-col h-full bg-gray-50 dark:bg-[#09090b] p-6 overflow-y-auto">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8 h-96">
                <div className="lg:col-span-2 h-full"><DashboardChart title="Hourly Order Activity" options={ordersHourly} height="100%" /></div>
                <div className="h-full"><DashboardChart title="Orders by Status" options={ordersByStatus} height="100%" /></div>
            </div>
