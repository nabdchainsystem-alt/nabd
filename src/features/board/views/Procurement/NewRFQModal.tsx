...
export const NewRFQModal: React.FC<NewRFQModalProps> = ({ isOpen, onClose, onSubmit, requestData }) => {
    ...
    const hasAnyPrice = React.useMemo(
        () => items.some(item => (item.unitPrice || 0) > 0),
        [items]
    );
    ...
    useEffect(() => {
        if (isOpen && requestData) {
            setVendorName('');
            const mappedItems: ItemWithPrice[] = (requestData.items || []).map((item: any) => ({
                    ...
                    <div className="grid grid-cols-1 gap-6">
                        ...
                        <div className="space-y-3">
                            <div className="flex justify-between items-center">
                                <div className="flex items-center gap-2">
                                    <label className="text-[11px] font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Items & Pricing</label>
                                    <span className="text-[11px] text-gray-400">Currency: SAR</span>
                                </div>
                                <div className="text-[11px] text-gray-500 dark:text-gray-400 flex items-center gap-1">
                                    <Package size={12} /> Qty from request: <span className="font-semibold text-gray-900 dark:text-gray-100">{requestTotalQty}</span>
                                </div>
                            </div>
                        ...
                        <div className="p-5 bg-gray-100 dark:bg-gray-800/50 rounded-xl border border-gray-200 dark:border-gray-700 space-y-3">
                            <div className="flex justify-between items-center text-sm">
                                ...
                                <span className="font-bold text-gray-900 dark:text-gray-100">{subtotal.toLocaleString(undefined, { minimumFractionDigits: 2 })}</span>
                            </div>
                            <div className="flex justify-between items-center text-sm">
                                <div className="flex items-center gap-2">
