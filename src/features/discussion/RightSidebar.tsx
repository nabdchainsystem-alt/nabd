...
import { remindersService, Reminder } from '../board/services/remindersService';
import { taskService } from '../board/services/taskService';
import { ITask as Task, Status, Priority } from '../board/types/boardTypes';
    ...
    return (
        ...
        >
            <div className="flex items-start gap-3">
                ...
                <button onClick={() => onToggleStatus(task)} className={`mt-0.5 transition-colors ${task.status === Status.Done ? 'text-green-500 hover:text-green-600' : 'text-gray-400 hover:text-gray-600'}`}>
                    {task.status === Status.Done ? <CheckCircle2 size={16} /> : <Circle size={16} />}
                </button>
                <div className="flex-1 min-w-0 pr-6">
                    <h4 className={`text-sm font-medium text-gray-900 truncate ${task.status === Status.Done ? 'line-through text-gray-400' : ''}`}>
                        {task.name}
                    </h4>
...
export const RightSidebar: React.FC<RightSidebarProps> = ({ className = '', contextId }) => {
                            ...
                            {showDatePicker && (
                                ...
                                >
                                    <SharedDatePicker
                                        ...
                                        onSelectDate={(date) => {
                                            setNewItemDate(date.toISOString());
    ...
    const handleDeleteTask = (task: Task, e: React.MouseEvent) => {
        e.stopPropagation();
        setItemToDelete({ id: task.id, type: 'task', title: task.name });
    };
    ...
    const toggleTaskStatus = async (task: Task) => {
        const nextStatus = task.status === Status.Done ? Status.New : Status.Done;
    ...
    const handleDragEnd = async (event: DragEndEvent) => {
        ...
        if (containerId === 'todo-section') newStatus = Status.New;
        else if (containerId === 'processing-section') newStatus = Status.Working;
        else if (containerId === 'done-section') newStatus = Status.Done;
    ...
    const todoTasks = safeTasks.filter(t => t.status === Status.New);
    const processingTasks = safeTasks.filter(t => t.status === Status.Working || t.status === Status.Stuck);
    const doneTasks = safeTasks.filter(t => t.status === Status.Done);
