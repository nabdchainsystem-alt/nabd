        ...
        <div className="h-full w-full flex flex-col gap-4 py-4">
            <div className="flex flex-wrap items-center justify-between gap-3">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <p className="text-[11px] uppercase tracking-[0.2em] text-slate-500 font-semibold">Goals &amp; OKRs</p>
                        <p className="text-sm text-slate-700 dark:text-slate-200 font-medium">Align work with simple objectives and measurable KRs.</p>
                    </div>
                </div>
            </div>

            {/* Add objective */}
            <div className="bg-white dark:bg-[#0f172a] border border-slate-200 dark:border-slate-800 rounded-xl p-4 shadow-sm flex flex-col gap-4">
                ...
                <div className="flex items-center justify-between text-xs text-slate-500 dark:text-slate-400">
                    <div className="flex items-center gap-1">
                        ...
                        <span>Key results stay simple and manual to keep focus.</span>
                    </div>
                    <button
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                {objectives.map((objective) => (
                    <div key={objective.id} className="bg-white dark:bg-[#0f172a] border border-slate-200 dark:border-slate-800 rounded-xl p-5 shadow-sm flex flex-col gap-4">
                        <div className="flex items-start justify-between gap-4">
                            <div>
                                <p className="text-[11px] uppercase tracking-[0.2em] text-slate-500 font-semibold">{objective.timeframe}</p>
                                <h3 className="text-lg font-semibold text-slate-900 dark:text-slate-50">{objective.title}</h3>
                                <p className="text-xs text-slate-500 flex items-center gap-1 mt-1">
                                    <User size={12} /> {objective.owner || 'Unassigned'}
                                </p>
                            </div>
                            <div className="flex flex-col items-end">
                                <span className="text-[11px] font-semibold text-slate-500">Progress</span>
                                <div className="text-2xl font-semibold text-slate-800 dark:text-slate-100">{progressForObjective(objective)}%</div>
                            </div>
                        </div>

                        <div className="space-y-3">
                            {objective.keyResults.map((kr) => (
                                <div key={kr.id} className="p-3 rounded-lg border border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-[#0f172a]">
                                    <div className="flex items-center justify-between gap-2">
                                        <div className="text-sm font-semibold text-slate-800 dark:text-slate-100">{kr.title}</div>
                                    ...
                                    <div className="mt-2 flex items-center gap-2">
                                        <select
                                            ...
                                            defaultValue=""
                                            className="flex-1 text-xs rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-[#0f172a] px-2 py-1"
                                        >
                                            <option value="">Link existing work item…</option>
                                    ...
                                    {kr.links.length > 0 && (
                                        <div className="mt-2 flex flex-wrap gap-1">
                                            {kr.links.map((link) => (
                                                <span key={link} className="text-[11px] px-2 py-1 rounded-full bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-200">
                                                    {link}
                                                </span>
                                            ))}
                                        </div>
                        ...
                        <div className="flex items-center gap-2">
                            <input
                                ...
                                className="flex-1 rounded-lg border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-[#111827] text-sm px-3 py-2 focus:outline-none"
                            />
                            <input
                                type="number"
                                ...
                                className="w-20 rounded-lg border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-[#111827] text-sm px-3 py-2 focus:outline-none"
                            />
                            <input
                                ...
                                className="w-16 rounded-lg border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-[#111827] text-sm px-2 py-2 focus:outline-none text-center"
                            />
                            <button
            ...
            {objectives.length === 0 && <div className="text-sm text-gray-500">No objectives yet. Add one to start tracking simple OKRs.</div>}
        </div>
    );
};
