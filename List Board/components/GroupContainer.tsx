...
import { GroupData, TaskItem, Status, ColumnWidths } from '../types';
...
export const GroupContainer: React.FC<GroupContainerProps> = ({ 
  ...
  const handleGenerateTasks = async () => {
    ...
    const readyTasks: TaskItem[] = newTasks.map(t => ({
      ...
      person: null,
      status: t.status || Status.WORKING_ON_IT,
  ...
  // Stats for footer
  const workingCount = group.items.filter(i => i.status === Status.WORKING_ON_IT).length;
  const doneCount = group.items.filter(i => i.status === Status.DONE).length;
  const stuckCount = group.items.filter(i => i.status === Status.STUCK).length;
  const total = group.items.length;
  
  const workingWidth = total ? (workingCount / total) * 100 : 0;
  const doneWidth = total ? (doneCount / total) * 100 : 0;
  const stuckWidth = total ? (stuckCount / total) * 100 : 0;
            ...
            <div className="flex h-[36px] border-b border-gray-200">
                ...
                <div className="bg-white px-2 py-2" style={{ width: colWidths.status }}>
                    <div className="w-full h-full flex rounded overflow-hidden">
                       {workingWidth > 0 && <div className="h-full bg-[#fdab3d]" style={{ width: `${workingWidth}%` }}></div>}
                       {doneWidth > 0 && <div className="h-full bg-[#00c875]" style={{ width: `${doneWidth}%` }}></div>}
                       {stuckWidth > 0 && <div className="h-full bg-[#e2445c]" style={{ width: `${stuckWidth}%` }}></div>}
                       {(total === 0 || (workingWidth + doneWidth + stuckWidth) < 100) && <div className="h-full bg-[#c4c4c4]" style={{ width: `${100 - (workingWidth + doneWidth + stuckWidth)}%` }}></div>}
                    </div>
                </div>
