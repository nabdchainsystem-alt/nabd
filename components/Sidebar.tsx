...
interface SidebarProps {
    ...
    onAddBoard: (name: string, icon: string) => void;
}
...
export const Sidebar: React.FC<SidebarProps> = ({
    ...
    const handleCreateBoard = (e: React.FormEvent) => {
        ...
        if (newBoardName.trim()) {
            onAddBoard(newBoardName, newBoardIcon);
    ...
    return (
        ...
        >
            ...
            <div className={`py-3 space-y-0.5 ${isCollapsed ? 'px-2 items-center flex flex-col' : 'px-4'}`}>
                ...
                <button className={`flex items-center space-x-3 rtl:space-x-reverse w-full px-3 py-1 rounded-md hover:bg-white/40 dark:hover:bg-monday-dark-hover text-gray-700 dark:text-monday-dark-text transition-colors ${isCollapsed ? 'justify-center' : ''}`}>
                    <Grid size={18} />
                        ...
                        >
                            ...
                            {isBoardHovered && (
                                <div className="absolute top-0 left-full rtl:right-full rtl:left-auto ms-1 w-56 bg-white dark:bg-monday-dark-surface shadow-xl rounded-lg border border-gray-200 dark:border-monday-dark-border py-2 z-[110]">
                                    <div
                                        onClick={(e) => { e.stopPropagation(); setIsNewBoardModalOpen(true); setIsAddMenuOpen(false); }}
            ...
            {isNewBoardModalOpen && (
                <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-[70] backdrop-blur-sm">
                    <div className="bg-white dark:bg-monday-dark-surface rounded-lg shadow-2xl w-96">
                        <div className="p-4 border-b border-gray-100 dark:border-monday-dark-border flex justify-between items-center">
                            <h3 className="font-semibold text-gray-800 dark:text-monday-dark-text">{t('new_board')}</h3>
                            <button onClick={() => setIsNewBoardModalOpen(false)}><ChevronDown className="rotate-180 text-gray-500" size={20} /></button>
                        </div>
